...
namespace CapturePanorama
{
    public class CapturePanorama : MonoBehaviour
    {
        ...
        public void Update()
        {
            bool captureKeyPressed = Input.GetKeyDown(captureKey) || capture;
            capture = false;
        ...
        public IEnumerator CaptureScreenshotAsyncHelper(string filenameBase, bool async)
        {
            ...
            for (int i = 0; i < ilimit; i++)
            {
                ...
                foreach (Camera c in cameras)
                {
                    ...
                    camGos[0].transform.localPosition = cameraAnchor.transform.position;
                    camGos[0].transform.localRotation = cameraAnchor.transform.rotation;
                    Transform target = cameraTarget.transform;
                    target.transform.position = new Vector3(target.transform.position.x, cameraAnchor.transform.position.y, target.transform.position.z);
                    camGos[0].transform.LookAt(target);
